...
- 使用 _List_ 检查 `docs/SPEC-1-REQUIREMENTS.md` 是否存在
- 如果文件存在 *THEN* 使用 _Write_ 载入内容并输出简明概述；否则使用 _Write_ 初始化新的规范文档骨架
- 在完成概述后，邀请用户保持放松、发挥想象，描绘理想的系统完成形态，然后 WAIT 用户输入以进入 [STEP-2]
...
- 若暂未筛选出可写入的要点，则使保持沉默，继续倾听直到获得明确信息
...
**编写指南：**
- 使用二级标题 `## Introduction`
- 描述应以 "此文档记录了开发者在开发" 开头，以 `$1` 语言进行书写
- 简明扼要地说明项目类型和主要目标
- 长度控制在 2-5 句话内
...
- 使用中文描述用户群体，多个群体用顿号分隔
- 描述应简洁、准确，与项目领域高度相关，且以 `$1` 语言书写
- 避免主观评价或艺术化表达
...
- **性能**：响应时间、并发处理、缓存策略等
- **可用性**：系统稳定、容灾备份、监控告警等
- **兼容性**：浏览器支持、设备适配、版本兼容等
- **可维护性**：代码规范、文档齐全、易于扩展等
- **部署运维**：容器化、自动化部署、监控体系等
...
1. **标准格式**：严格遵循了文档命名、结构和内容要求
2. **用户导向**：每个需求都从用户角度出发，明确用户价值
3. **可测试性**：验收标准使用 Given-When-Then 格式，便于验证
4. **优先级管理**：使用 [H]/[M]/[L] 标识需求优先级
5. **完整性**：涵盖了功能、约束、非功能性需求和后续规划
