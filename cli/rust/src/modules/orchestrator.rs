...
impl Orchestrator {
        ...
        for dir in directories {
            ...
            if !dir_path.exists() {
                std::fs::create_dir_all(dir_path).map_err(|e| {
                    AgentError::Storage(format!("Failed to create directory {}: {}", dir, e))
                })?;
    ...
    pub fn load_config(&mut self) -> Result<()> {
        ...
        self.config = serde_json::from_str(&config_content).map_err(|e| {
            AgentError::Configuration(format!("Failed to parse configuration: {}", e))
        })?;
        Ok(())
    }
