...
impl StorageManager {
    ...
    pub fn new<P: AsRef<Path>>(base_path: P) -> Result<Self> {
        ...
        fs::create_dir_all(&backup_path).map_err(|e| {
            AgentError::Storage(format!("Failed to create backup directory: {}", e))
        })?;
    ...
    pub fn write_file_atomic<P: AsRef<Path>, C: AsRef<[u8]>>(
        ...
        if let Some(parent) = full_path.parent() {
            fs::create_dir_all(parent).map_err(|e| {
                AgentError::Storage(format!("Failed to create parent directory: {}", e))
            })?;
        }
    ...
    pub fn create_backup(&self, description: Option<String>) -> Result<BackupInfo> {
        ...
        for file in &files {
            if let Ok(relative_path) = file.strip_prefix(&self.base_path) {
                ...
                fs::copy(file, &backup_file_path).map_err(|e| {
                    AgentError::Storage(format!("Failed to copy file to backup: {}", e))
                })?;
            }
        }
    ...
    pub fn restore_from_backup(&self, backup: &BackupInfo) -> Result<()> {
        ...
        for file in &backup.files {
            ...
            if current_file_path.exists() {
                fs::remove_file(&current_file_path).map_err(|e| {
                    AgentError::Storage(format!("Failed to remove existing file: {}", e))
                })?;
            }
    ...
    pub fn list_backups(&self) -> Result<Vec<BackupInfo>> {
        ...
        {
            let entry = entry
                .map_err(|e| AgentError::Storage(format!("Failed to read backup entry: {}", e)))?;
            let path = entry.path();
                    ...
                    && let Ok(timestamp) = timestamp_str.parse::<u64>()
                {
                    ...
                    let backup_info = if metadata_path.exists() {
                        let content = fs::read_to_string(&metadata_path).map_err(|e| {
                            AgentError::Storage(format!("Failed to read backup metadata: {}", e))
                        })?;
                        serde_json::from_str(&content).map_err(|e| {
                            AgentError::Storage(format!("Failed to parse backup metadata: {}", e))
                        })?
                    } else {
